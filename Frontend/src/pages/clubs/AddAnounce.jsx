...
const imageSchema = yup
  ...
  .test('fileSize', 'File size is too large', (value) => {
    return value === null || (value instanceof File && value.size <= 1024 * 1024); // 1MB limit
  });
...
const AddAnouce = ({ anouce, setAnouce }) => {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const onSubmit = (data) => {
    console.log('Form data:', { ...data, previewImage });
    setAnouce(false);
  };
  ...
  return (
    <div className="w-full h-[450%] bg-[#0000008a] absolute z-100 top-0 left-0 right-0 pt-40">
      <div
        style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', boxShadow: '0 2px 4px rgba(0, 0, 0, 0.1)' }}
